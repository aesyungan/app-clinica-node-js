...
//optenerAll
router.get('/', (req, res) => {
    db.AntecedentesPersonales.findAll().then(data => {
...
//optener ID
router.get('/:listado_antecedente_id/:paciente_id', (req, res) => {
    db.AntecedentesPersonales.findOne({
        where: {
            paciente_id: req.params.paciente_id,
            listado_antecedente_id: req.params.listado_antecedente_id
        }
    }
    ).then(data => {
...
router.post('/', (req, res) => {
    ...
    db.AntecedentesPersonales.create(req.body).then(data => {
...
router.put('/', (req, res) => {
    // console.log("test->");
    // console.log(req.body);
    db.AntecedentesPersonales.update(req.body, { where: { id: req.body.id } })
        .then(data => {
            if (data[0] == 1) {
...
//eliminar
router.delete('/:listado_antecedente_id/:paciente_id', (req, res) => {
    // console.log("test->");
    // console.log(req.body);
    req.params.id;
    db.AntecedentesPersonales.destroy({
        where: {
            paciente_id: req.params.paciente_id,
            listado_antecedente_id: req.params.listado_antecedente_id
        }
