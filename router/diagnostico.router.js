...
//optenerAll
router.get('/', (req, res) => {
    db.Diagnostico.findAll().then(data => {
...
router.post('/', (req, res) => {
    ...
    db.Diagnostico.create(req.body).then(data => {
...
router.put('/', (req, res) => {
    // console.log("test->");
    // console.log(req.body);
    db.Diagnostico.update(req.body, { where: { id: req.body.id } })
        .then(data => {
            if (data[0] == 1) {
